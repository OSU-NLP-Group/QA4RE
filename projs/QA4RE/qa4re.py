...
def call_gpt_engine_df(args, input_ready_df, prompt_params, output_dir):
    """call openai gpt models with examples in the input_ready_df."""
    open_ai_responses = []
    predictions = []
    engine_generations = []
    time_list = []
    for i, row in tqdm(input_ready_df.iterrows()):
        ...
        # call gpt engine
        response = call_gpt_engine(args.engine, final_input_prompt, logit_biases, num_logprobs=len(all_choices), **prompt_params)
        generation, prediction = get_prediction(args, prompt_params, response, all_choices, correct_choices, index2rel, args.engine, first_token_of_each_verb_label)
        engine_generations.append(generation)
        open_ai_responses.append(response)
        predictions.append(prediction)
        time_list.append(time.time() - time_start)

    input_ready_df['open_ai_responses'] = open_ai_responses
    input_ready_df['engine_generations'] = engine_generations
...
def run_qa4re(args):
    ...
    check_example(input_ready_df, ['final_input_prompts', 'all_choices', 'correct_choices', 'index2rels', 'engine_generations', 'predictions'])
