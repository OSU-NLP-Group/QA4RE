...
def call_gpt_engine_df(args, input_ready_df, prompt_params, output_dir):
    ...
    for i, row in tqdm(input_ready_df.iterrows()):
        ...
        # call gpt engine
        response = call_gpt_engine(args.engine, final_input_prompt, logit_biases, **prompt_params)
        generation, prediction = get_prediction(prompt_params, response, verbalized_labels, args.engine, first_token_of_each_verb_label)
...
def run_vanilla_re(args):
    ...
    check_example(output_df, ['final_input_prompts', 'engine_generations', 'predictions'])
